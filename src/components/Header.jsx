...
// Original colors backed up in LOGO_COLORS_BACKUP.js
// Original: Red #ee2e3a, Blue #243e87
// Current: Company Blue #243E87 (light), White (dark)
const Logo = ({ className, ariaLabel }) => (
  <svg
  ...
  >
    <g>
      <path
        className="[data-color='1'] fill-[#EE2E3A] dark:fill-[#EE2E3A]"
        fill="#243E87"
        d="M271.79 438.93v9.46H159.96v-9.46h111.83z"
        data-color="1"
      />
      <path
        className="[data-color='2'] fill-[#243E87] dark:fill-[#ECE4CF]"
        fill="#243E87"
      ...
      <path
        className="[data-color='2'] fill-[#243E87] dark:fill-[#ECE4CF]"
        fill="#243E87"
        d="M322.25 440.23l-2.74-.49c.31-1.1.84-1.92 1.59-2.45.75-.53 1.87-.79 3.35-.79 1.34 0 2.35.16 3 .48.66.32 1.12.72 1.39 1.21.27.49.4 1.39.4 2.7l-.03 3.52c0 1 .05 1.74.14 2.22.1.48.28.99.54 1.53h-2.98c-.08-.2-.18-.5-.29-.89-.05-.18-.09-.3-.11-.35-.52.5-1.07.88-1.65 1.13-.59.25-1.21.38-1.88.38-1.17 0-2.1-.32-2.77-.95-.68-.64-1.01-1.44-1.01-2.41c0-.64.15-1.22.46-1.72.31-.5.74-.89 1.29-1.16.55-.27 1.35-.5 2.4-.7 1.41-.26 2.39-.51 2.93-.74v-.3c0-.58-.14-.99-.43-1.24s-.83-.37-1.62-.37c-.54 0-.95.11-1.26.32-.29.17-.53.54-.72 1.07zm4.04 2.45c-.39.13-1 .28-1.83.46-.84.18-1.38.35-1.64.53-.39.28-.59.63-.59 1.06 0 .42.16.79.47 1.09.31.31.72.46 1.2.46.54 0 1.06-.18 1.56-.54.36-.27.6-.6.72-1 .08-.26.12-.75.12-1.47v-.59z"
        data-color="2"
      />
      <path
        className="[data-color='2'] fill-[#243E87] dark:fill-[#ECE4CF]"
...
const Header = () => {
  ...
  return (
    <header
      className="relative w-full bg-[var(--parchment-light)]/90 dark:bg-[#06140f]/90 backdrop-blur-lg px-4 lg:px-8 py-4 transition-all duration-300"
      dir={isArabic ? "rtl" : "ltr"}
    >
      {/* Double gold rule along the bottom edge */}
      <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-[var(--chrome-border)]" />
      <div className="absolute bottom-[3px] left-0 right-0 h-[1px] bg-[var(--chrome-border)]/50" />

      <div className="container mx-auto max-w-6xl flex flex-col sm:flex-row items-center justify-between gap-4 relative z-10">
        <Link to={occasion ? "/welcome" : "/"} aria-label="Home" className="group">
          <div className="rounded-sm p-1 transition-transform duration-300 group-hover:scale-105">
            <Logo
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap justify-center">
          {/* Change Occasion — hidden while only one occasion is active.
              Re-enable when multiple occasions are reactivated.
          {occasion && (
            <button
              onClick={handleChangeOccasion}
              className={`inline-flex items-center gap-2 px-3.5 py-2 text-xs uppercase tracking-[0.18em] text-[var(--chrome-text)] border border-[var(--chrome-border)] hover:bg-[var(--chrome-border)]/10 rounded-sm transition-all duration-300 ${labelFont}`}
              aria-label={t("change_occasion")}
            >
              <RefreshCw className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">{t("change_occasion")}</span>
            </button>
          )}
          */}

          {/* Visit Website — brand sapphire action */}
          <a
            href="https://www.redahazardcontrol.com/"
            target="_blank"
            rel="noopener noreferrer"
            className={`inline-flex items-center gap-2 px-4 py-2 text-xs uppercase tracking-[0.18em] rounded-sm text-white bg-gradient-to-br from-[var(--brand-sapphire)] to-[var(--brand-sapphire-deep)] border border-[var(--chrome-border)] shadow-md hover:shadow-[0_8px_20px_rgba(36,62,135,0.4)] dark:hover:shadow-[0_8px_20px_rgba(74,107,201,0.35)] transition-all duration-300 ${labelFont}`}
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`group inline-flex items-center gap-2 px-3 py-2 rounded-sm border border-[var(--chrome-border)] hover:bg-[var(--chrome-border)]/10 transition-all duration-300 ${labelFont}`}
            aria-label={
          ...
          >
            <EightPointStar
              size={10}
              className="text-[var(--chrome-text)] opacity-70 group-hover:opacity-100"
            />
            <span className="text-xs uppercase tracking-[0.18em] text-[var(--chrome-text)]">
...
export { Logo }; // Export Logo for reuse in other components
