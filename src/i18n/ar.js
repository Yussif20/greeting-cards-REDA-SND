// UI chrome only. Occasion and design copy lives in src/data/*.

export default {
  common: {
    appName: "بطاقات رضا",
    tagline: "معاً في كل مناسبة",
    taglineAr: "بطاقات معايدة رسمية لمختلف المناسبات على مدار العام",
    breadcrumb: { home: "الرئيسية" },
    language: "اللغة",
    theme: { light: "الوضع الفاتح", dark: "الوضع الداكن", label: "المظهر" },
    loading: "جارٍ التحميل",
    dismiss: "إغلاق",
  },

  home: {
    title: "بطاقات رضا",
    subtitle: "معاً في كل مناسبة",
    intro: "بطاقات معايدة رسمية لمختلف المناسبات على مدار العام.",
    openOccasion: "فتح {{name}}",
  },

  brands: {
    chooseABrand: "اختر الشركة",
    open: "فتح {{name}}",
    // Not a plural key -- see the note on the English side.
    cardCount: "{{count}} بطاقة",
    empty: "لا توجد بطاقات بعد",
    noArtwork: "لا يوجد تصميم بعد",
    other: "بطاقات أخرى",
    none: "لا توجد بطاقات لهذه المناسبة بعد.",
    noCards: "لا توجد بطاقات لهذه الشركة في هذا الموسم.",
    backToBrands: "اختر شركة أخرى",
  },

  designs: {
    heading: "اختر التصميم",
    chooseADesign: "اختر التصميم",
    category: "التصنيف",
    allCategories: "الكل",
    year: "السنة",
    loadMore: "عرض المزيد من التصاميم",
    selected: "محدد",
    design: "تصميم",
    empty: "لا توجد تصاميم مطابقة لهذا التصنيف.",
    clearFilter: "عرض كل التصاميم",
    sampleArtwork: "عينة تصميم",
    sampleNotice:
      "تصاميم هذه المناسبة قيد الإعداد. البطاقات المعروضة عينات مستعارة وتحمل تهنئة مناسبة أخرى.",
    style: {
      all: "الكل",
      modern: "عصري",
      traditional: "تقليدي",
      minimal: "بسيط",
      elegant: "أنيق",
    },
  },

  editor: {
    title: "تخصيص بطاقتك",
    subtitle: "خصص بطاقتك لتناسبك",
    optional: "اختياري",
    preview: "معاينة البطاقة",
    field: {
      name: "الاسم",
      jobTitle: "المسمى الوظيفي",
      brand: "الشركة",
      font: "الخط",
      textColor: "لون النص",
    },
    placeholder: {
      name: "أدخل اسمك",
      jobTitle: "أدخل مسماك الوظيفي",
    },
    fontHelp:
      "يُطبَّق الخط على الاسم والمسمى الوظيفي معاً. وكل الخيارات تدعم العربية واللاتينية.",
    brandUnavailable: "لا توجد بطاقة لهذه الشركة في هذه المناسبة",
    tool: {
      move: "تحريك",
      size: "الحجم",
      align: "المحاذاة",
    },
    align: {
      left: "يسار",
      center: "توسيط",
      right: "يمين",
      top: "أعلى",
      middle: "منتصف",
      bottom: "أسفل",
      distribute: "توزيع متساوٍ",
    },
    size: { label: "الحجم", nudge: "استخدم مفاتيح الأسهم لتحريك الطبقة المحددة" },
    action: {
      reset: "إعادة تعيين",
      saveDraft: "حفظ المسودة",
      download: "تحميل",
      share: "مشاركة",
    },
    draftSaved: "تم حفظ المسودة",
    draftRestored: "تمت استعادة المسودة",
    discardDraft: "تجاهل",
    resetDone: "تمت إعادة تعيين البطاقة",
    enterNameFirst: "أدخل الاسم أولاً",
    selectLayerHint: "انقر على البطاقة لتحديد النص وتحريكه",
  },

  errors: {
    notFoundTitle: "الصفحة غير موجودة",
    notFoundBody: "هذه الصفحة غير موجودة. سنعيدك إلى البداية.",
    backHome: "العودة إلى المناسبات",
    imageLoad: "تعذر تحميل هذا التصميم. الرجاء اختيار تصميم آخر.",
    download: "تعذر تحميل البطاقة. الرجاء المحاولة مرة أخرى.",
    share: "تعذرت مشاركة البطاقة. الرجاء المحاولة مرة أخرى.",
    unknownOccasion: "تعذر العثور على هذه المناسبة.",
  },

  footer: {
    tagline: "معاً في كل مناسبة",
  },

  admin: {
    title: "لوحة تحكم بطاقات ريدة",
    loading: "جارٍ التحميل…",
    retry: "إعادة المحاولة",

    auth: {
      subtitle: "سجّل الدخول لإدارة المناسبات والبطاقات.",
      email: "البريد الإلكتروني",
      password: "كلمة المرور",
      signIn: "تسجيل الدخول",
      signOut: "تسجيل الخروج",
      invalid: "البريد الإلكتروني أو كلمة المرور غير صحيحة. الرجاء المحاولة مرة أخرى.",
      notAdminTitle: "هذا الحساب ليس حساب مسؤول",
      notAdminBody:
        "تم تسجيل دخولك، لكن هذا الحساب لا يملك صلاحية الإدارة. اطلب من المسؤول عن الموقع إضافتك.",
      unconfiguredTitle: "لوحة التحكم غير مهيأة",
      unconfiguredBody:
        "المتغيران VITE_SUPABASE_URL والمفتاح العام غير موجودين في هذه النسخة. راجع ملف .env.example.",
    },

    nav: {
      label: "أقسام لوحة التحكم",
      occasions: "المناسبات",
      designs: "البطاقات",
      categories: "التصنيفات",
      fonts: "الخطوط",
      seasons: "المواسم",
      viewSite: "عرض الموقع",
    },

    status: {
      published: "منشورة",
      draft: "مسودة",
      archived: "مؤرشفة",
    },

    occasions: {
      delete: "حذف",
      deleted: "تم حذف المناسبة.",
      confirmDelete: "حذف هذه المناسبة نهائيًا؟ لم تُنشر من قبل، لذا لن ينكسر أي رابط.",
      confirmDeletePublished:
        "كانت هذه المناسبة منشورة. حذفها نهائيًا يعني توقّف أي رابط تمت مشاركتها عن العمل. هل تريد الحذف؟",
      hasDesigns:
        "لا تزال هذه المناسبة تضم {{count}} بطاقة. احذفها أو انقلها أولًا — حذف المناسبة الآن سيأخذ تصاميمها معها.",
      add: "إضافة مناسبة",
      edit: "تعديل",
      moveUp: "تحريك لأعلى",
      moveDown: "تحريك لأسفل",
      title: "المناسبات",
      subtitle: "كل ما يمكن للموقع عرضه، بما في ذلك ما لم يُنشر بعد.",
      empty: "لا توجد مناسبات بعد.",
      hidden: "مخفية عن الصفحة الرئيسية",
      designCount: "{{count}} بطاقة",
      draftCount: "{{count}} غير منشورة",
    },

    designs: {
      add: "إضافة بطاقة",
      layout: "التخطيط",
      publish: "نشر",
      archive: "إلغاء النشر",
      delete: "حذف",
      publishedToast: "البطاقة منشورة الآن.",
      archived: "لم تعد البطاقة ظاهرة للعامة.",
      deleted: "تم حذف المسودة.",
      confirmDeletePublished:
        "كانت هذه البطاقة منشورة. حذفها نهائيًا يعني توقّف أي رابط تمت مشاركته لها عن العمل. هل تريد الحذف؟",
      confirmDelete: "حذف هذه المسودة نهائيًا؟ لم تُنشر من قبل، لذا لن ينكسر أي رابط.",
      title: "البطاقات",
      subtitle: "التصاميم لكل مناسبة وكل موسم.",
      occasion: "المناسبة",
      allOccasions: "كل المناسبات",
      empty: "لا توجد بطاقات لهذه المناسبة بعد.",
      placeholder: "تصميم مؤقت",
      category: "التصنيف",
      noCategory: "بدون تصنيف",
      categorySaved: "تم حفظ التصنيف.",
    },

    errors: {
      loadFailed: "تعذر تحميل البيانات من قاعدة البيانات.",
    },

    upload: {
      title: "إضافة بطاقة",
      subtitle: "ارفع التصميم، ثم حدّد موضع الاسم عليه.",
      occasion: "المناسبة",
      season: "الموسم",
      brand: "الشركة",
      category: "التصنيف",
      noCategory: "بدون تصنيف",
      style: "النمط",
      choose: "اختر صورة أو أفلتها هنا",
      hint: "JPEG أو PNG أو WebP",
      cancel: "إلغاء",
      note: "تُحفظ البطاقة كمسودة. لا يراها أحد حتى تنشرها.",
      busy: {
        processing: "جارٍ تغيير حجم التصميم…",
        uploading: "جارٍ الرفع…",
        saving: "جارٍ الحفظ…",
      },
      errors: {
        badType: "هذا الملف ليس بصيغة JPEG أو PNG أو WebP.",
        tooLarge: "حجم الملف كبير جدًا. اجعله أقل من ٤٠ ميجابايت.",
        tooManyPixels: "عدد بكسلات هذه الصورة أكبر من أن تُعالَج في المتصفح.",
        decodeFailed: "تعذّرت قراءة هذه الصورة.",
        encodeFailed: "تعذّر على المتصفح ترميز الصورة بعد تغيير حجمها.",
        unsupportedFormat: "هذا المتصفح لا يستطيع إنشاء صور مصغّرة بصيغة WebP.",
        blankCanvas: "نفدت ذاكرة المتصفح فأنتج صورة فارغة. جرّب ملفًا أصغر.",
      },
    },

    layout: {
      palette: "ألوان النص",
      paletteHint: "ما يمكن للعميل الاختيار منه. اللون المحاط بإطار هو الافتراضي.",
      swatch: "اللون {{colour}}",
      addSwatch: "إضافة لون",
      removeSwatch: "إزالة {{colour}}",
      regions: "المنطقة الآمنة وموضع الشعار",
      regionsHint:
        "المنطقة الآمنة هي ما تستند إليه أدوات المحاذاة. وموضع الشعار هو الجزء الذي يُقتطع لعرض شعار الشركة.",
      safeArea: "المنطقة الآمنة",
      brandMark: "موضع الشعار",
      title: "تحديد موضع النص",
      save: "حفظ التخطيط",
      saveAndPublish: "حفظ ونشر",
      saved: "تم حفظ التخطيط.",
      published: "تم الحفظ والنشر.",
      sampleName: "اسم تجريبي",
      sampleTitle: "مسمى وظيفي تجريبي",
      sampleHint: "نص تجريبي فقط — لا يُحفظ أبدًا. جرّب اسمًا طويلًا للتأكد من اتساعه.",
      showLogo: "إظهار موضع الشعار",
    },

    occasion: {
      newTitle: "مناسبة جديدة",
      editTitle: "تعديل المناسبة",
      subtitle: "شكل البطاقة في الصفحة الرئيسية، واسمها.",
      create: "إنشاء",
      save: "حفظ",
      slug: "العنوان",
      slugHint: "ستكون صفحات البطاقات على ‎/{{slug}}‎. لا يمكن تغييره لاحقًا.",
      slugLocked: "ثابت بعد الإنشاء: معرّف كل بطاقة يتضمنه، والروابط المشاركة تعتمد عليه.",
      title: "الاسم",
      shortTitle: "الاسم المختصر",
      tagline: "الوصف",
      defaultFont: "خط البطاقات الافتراضي",
      defaultFontHint:
        "يظهر أولًا في كل بطاقات هذه المناسبة. لا يمكن اختيار الخطوط المسودة أو المؤرشفة.",
      cardFont: "استخدام خط كل بطاقة",
      hero: "صورة البطاقة",
      heroAlt: "وصف الصورة",
      focalX: "التركيز الأفقي",
      focalY: "التركيز الرأسي",
      focalHint: "أي جزء من الصورة يبقى ظاهرًا عند اقتصاص البطاقة.",
      brandCovers: "صور الشركات",
      brandCoversHint:
        "الصورة التي تمثّل كل شركة في صفحة هذه المناسبة. بدونها تُعرض أول بطاقة لتلك الشركة.",
      coverFallback: "تُعرض حاليًا أول بطاقة لهذه الشركة.",
      coverChoose: "اختر صورة {{name}}",
      coverReplace: "استبدال صورة {{name}}",
      coverRemove: "إزالة",
      icon: "الرمز",
      iconSearch: "بحث في الرموز",
      iconNone: "لا توجد رموز مطابقة.",
      iconHandDrawn: "لهذه المناسبة رمز مرسوم خصيصًا لها ({{name}}).",
      iconGroup: {
        celebration: "احتفال",
        sky: "سماء",
        heritage: "تراث",
        nature: "طبيعة",
        journey: "رحلة",
        other: "أخرى",
      },
      accent: { light: "اللون المميز (فاتح)", dark: "اللون المميز (داكن)" },
      accentHint: "تُشتق بقية الألوان من هذين اللونين، بما في ذلك التدرّج خلف العنوان.",
      preview: "معاينة",
      previewEmpty: "أضف صورة لرؤية البطاقة.",
      reordered: "تم حفظ الترتيب.",
      published: "المناسبة منشورة الآن.",
      archived: "لم تعد المناسبة ظاهرة للعامة.",
    },
    seasons: {
      title: "المواسم",
      subtitle: "كل سنة من التصاميم، تُحفظ ولا تُستبدل.",
      add: "إضافة موسم",
      create: "إنشاء",
      id: "الموسم",
      idHint: "سنتان، مثل 2026-2027. لا يمكن تغييره لاحقًا.",
      labelEn: "التسمية (بالإنجليزية)",
      labelAr: "التسمية (بالعربية)",
      empty: "لا توجد مواسم بعد.",
      cardCount: "{{count}} بطاقة",
      created: "تم إنشاء الموسم كمسودة.",
      published: "الموسم منشور الآن.",
      archived: "لم يعد الموسم ظاهرًا للعامة.",
      deleted: "تم حذف الموسم.",
      confirmDelete: "حذف هذا الموسم نهائيًا؟",
      hasDesigns:
        "لا يزال هذا الموسم يضم {{count}} بطاقة. احذفها أو انقلها أولًا — حذف الموسم الآن سيأخذ تصاميمها معه.",
    },

    duplicate: {
      action: "تكرار",
      title: "تكرار هذه البطاقة",
      brand: "الشركة",
      category: "التصنيف",
      noCategory: "بدون تصنيف",
      newArtwork: "اسحب التصميم الجديد هنا",
      keepImage: "أو اتركه فارغًا لاستخدام نفس الصورة",
      clearFile: "إزالة",
      create: "تكرار",
      cancel: "إلغاء",
      created: "تم إنشاء {{id}} كمسودة.",
      aspectWarning: "أبعاد التصميم الجديد مختلفة، لذا راجع موضع النص.",
      hint: "تحتفظ النسخة بتخطيط هذه البطاقة ونمطها ومناسبتها وموسمها، وتُحفظ كمسودة. أما موضع الشعار فهو اقتصاص من هذا التصميم — أعد تحديده إذا كان شعار التصميم الجديد في مكان آخر.",
    },

    fonts: {
      title: "الخطوط",
      subtitle: "الخطوط المتاحة في محرر البطاقات، إضافةً إلى الأربعة المدمجة.",
      add: "إضافة خط",
      create: "رفع",
      cancel: "إلغاء",
      save: "حفظ",
      edit: "إعادة تسمية",
      id: "العنوان",
      idHint: "يُحفظ في كل بطاقة تستخدم هذا الخط. لا يمكن تغييره لاحقًا.",
      labelEn: "الاسم (بالإنجليزية)",
      labelAr: "الاسم (بالعربية)",
      regular: "عادي",
      bold: "عريض",
      required: "مطلوب",
      optional: "اختياري",
      dropFile: "اسحب ملف الخط هنا",
      fileHint: "TTF أو OTF أو WOFF أو WOFF2",
      clearFile: "إزالة",
      addBold: "إضافة العريض",
      boldAdded: "تمت إضافة الملف العريض.",
      oneWeight: "وزن واحد",
      twoWeights: "عادي وعريض",
      moveUp: "تحريك لأعلى",
      moveDown: "تحريك لأسفل",
      empty: "لا توجد خطوط مرفوعة بعد. الخطوط الأربعة المدمجة متاحة دائمًا.",
      created: "تم رفع الخط كمسودة.",
      saved: "تمت إعادة تسمية الخط.",
      published: "الخط منشور الآن.",
      archived: "لم يعد الخط ظاهرًا للعامة.",
      deleted: "تم حذف الخط.",
      reordered: "تم حفظ الترتيب.",
      confirmDelete:
        "حذف هذا الخط نهائيًا؟ أي بطاقة لا تزال تستخدمه ستعود إلى خط القاهرة.",
      hint: "الملف العادي مطلوب؛ وبإضافة ملف عريض تظهر أسماء البطاقات أثقل فعليًا، وإلا استخدمت البطاقة كلها وزنًا واحدًا. ولا يُصطنع العريض أبدًا. وما لا يغطيه الخط — اللاتينية في خط عربي فقط أو العكس — يعود إلى خط القاهرة. وحجم WOFF2 نحو نصف حجم TTF نفسه.",
      busy: {
        uploading: "جارٍ الرفع…",
        saving: "جارٍ الحفظ…",
      },
      errors: {
        badFontType: "هذا ليس ملف خط. استخدم TTF أو OTF أو WOFF أو WOFF2.",
        fontTooLarge: "حجم ملف الخط كبير جدًا. اجعله أقل من ٨ ميجابايت.",
        regularRequired: "أضف ملف الخط العادي أولًا.",
      },
    },

    categories: {
      title: "التصنيفات",
      subtitle: "الغرض من البطاقة — موظفين، عملاء، أو أي تصنيف تحتاجه لاحقًا.",
      add: "إضافة تصنيف",
      create: "إنشاء",
      id: "العنوان",
      idHint: "يظهر في الرابط على شكل ‎?category=‎. لا يمكن تغييره لاحقًا.",
      labelEn: "الاسم (بالإنجليزية)",
      labelAr: "الاسم (بالعربية)",
      edit: "إعادة تسمية",
      save: "حفظ",
      cancel: "إلغاء",
      moveUp: "تحريك لأعلى",
      moveDown: "تحريك لأسفل",
      empty: "لا توجد تصنيفات بعد. البطاقات بلا تصنيف تظهر ضمن «الكل».",
      cardCount: "{{count}} بطاقة",
      created: "تم إنشاء التصنيف كمسودة.",
      saved: "تمت إعادة تسمية التصنيف.",
      published: "التصنيف منشور الآن.",
      archived: "لم يعد التصنيف ظاهرًا للعامة.",
      deleted: "تم حذف التصنيف.",
      reordered: "تم حفظ الترتيب.",
      confirmDelete: "حذف هذا التصنيف نهائيًا؟ لم يُنشر من قبل، لذا لن ينكسر أي رابط.",
      confirmDeleteWithCards:
        "هناك {{count}} بطاقة مصنّفة ضمن هذا التصنيف. حذفه سيتركها بلا تصنيف، ولن تتأثر التصاميم. هل تريد الحذف؟",
      hint: "يمكن تصنيف كل بطاقة ضمن أحد هذه التصنيفات أو تركها بلا تصنيف. ولا يظهر التصنيف في الموقع إلا بعد نشره واستخدام بطاقة له.",
    },
  },
};
